...
namespace NetTopologySuite.IO.GeoJSON.Test.Issues.NetTopologySuite.IO.GeoJSON
{
  ...
  ""properties"": {
    ...
    ""kommunekode"": ""0101""
            ...
            Assert.That(f.Attributes.TryGetValue("id", out object id), Is.True);
  ...
  // no ""properties"" here!!
        ...
        public void TestWriteFeatureCollectionWithFirstFeatureGeometryNull()
        {
            ...
            var attributesTable = new Dictionary<string, object> { {"key", "value"}};
  ...
  // the end
        ...
        public void TestNullValueHandlingEnvelope()
        {
            ...
            Feature value = new Feature(factory.CreatePoint(new Coordinate(23, 56)), null);
        ...
        public void TestOutputDimension()
        {
            ...
            var serializer2 = GeoJsonSerializer.Create(factory, 2, 0);
            var serializer3 = GeoJsonSerializer.Create(factory, 3, 0);
            var writer = new StringWriter();
        ...
        public void TestInputDimension()
        {
            ...
            var serializerZ = GeoJsonSerializer.Create(factory, 3, 0);
            ...
            var serializer = GeoJsonSerializer.Create(factory, 2, 0);
            ...
            var serializerM = GeoJsonSerializer.Create(factory, 3, 1);
            reader = new JsonTextReader(new StringReader("{\"type\":\"Polygon\",\"coordinates\":[[[0.001,0.001,3.0],[10.1,0.002,3.0],[10.0,10.1,3.0],[0.05,9.999,3.0],[0.001,0.001,3.0]]]}"));
            geom = serializerM.Deserialize<Geometry>(reader);
            Assert.That(wktWriter.Write(geom), Is.EqualTo("POLYGON M((0 0 3, 10.1 0 3, 10 10.1 3, 0.1 10 3, 0 0 3))"));

            var serializerZM = GeoJsonSerializer.Create(factory, 4, 1);
            reader = new JsonTextReader(new StringReader("{\"type\":\"Polygon\",\"coordinates\":[[[0.001,0.001,3.0,8],[10.1,0.002,3.0,9],[10.0,10.1,3.0,10],[0.05,9.999,3.0,11],[0.001,0.001,3.0,12]]]}"));
            geom = serializerZM.Deserialize<Geometry>(reader);
            Assert.That(wktWriter.Write(geom), Is.EqualTo("POLYGON ZM((0 0 3 8, 10.1 0 3 9, 10 10.1 3 10, 0.1 10 3 11, 0 0 3 12))"));
        }
        ...
        public void TestAddToSerializerSettings()
        {
            ...
            Assert.That(() =>
            {
                ...
                jss.Converters.Add(new GeometryConverter(factory, dimension, measures));
                jss.Converters.Add(new GeometryArrayConverter(factory, dimension, measures));
                jss.Converters.Add(new CoordinateConverter(factory.PrecisionModel, dimension, measures));
